...
\begin{itemize}
  ...
  repository.
  \item Commit-level review actions call \texttt{/commits/hf/review} from the
...
Both repositories keep automation artifacts inside the codebase. On the
backend, the \texttt{.github/agents/} directory defines dedicated roles such as
architect, implementer, reviewer, and QA agent.

On the frontend, dedicated \texttt{.github/} folders capture UI guidelines,
review rules, and reusable workflows for Vue, Quasar, styling, and quality
checks. Keeping these files versioned with the source code makes automation
